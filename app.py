...
import json
import re
import yaml  # Add yaml import for loading hardcoded ideas
import traceback
import sys
import logging
import click
...
# LLM-generated scientific hypotheses are inherently subjective and challenging to evaluate. Current LLM-based assessment methods fall short in capturing the nuanced scientific merit of novel ideas, while expert evaluations are resource-intensive. There is a need for a systematic, scalable framework to objectively benchmark and validate the value of scientific hypothesis. Such an approach would streamline the integration of innovative ideas into empirical research.
#
...
# Flag to control whether to use hardcoded ideas for the initial research brief
global USE_HARDCODED_IDEAS  # Set to False to use the ideation agent for all briefs
global HARDCODED_IDEA_KEY  # Which idea to use from ideas.yaml if hardcoded ideas are enabled

USE_HARDCODED_IDEAS = False  # Set to False to use the ideation agent for all briefs
HARDCODED_IDEA_KEY = 'default'

# Load hardcoded ideas from YAML file
hardcoded_ideas = {}
try:
    with open("config/ideas.yaml", "r") as f:
        ideas_config = yaml.safe_load(f)
        hardcoded_ideas = ideas_config.get('ideas', {})
except Exception as e:
    print(f"Error loading hardcoded ideas: {str(e)}")
...
# Function to get a hardcoded idea from the ideas config
def get_hardcoded_idea(key=None):
    """Get a hardcoded idea by key, or the default if key doesn't exist"""
    global hardcoded_ideas
    
    # If no key specified, use the default from config
    if key is None:
        key = HARDCODED_IDEA_KEY
    
    # Return the requested idea or the default if not found
    return hardcoded_ideas.get(key, hardcoded_ideas.get('default', 'No hardcoded ideas available.'))
...
def chat():
    global main_idea, current_root, current_node, USE_HARDCODED_IDEAS
    if request.method == "GET":
        ...
        try:
            ...
            if current_root is None:
                ...
                # G1ither hardcoded or from ideation agent
                if USE_HARDCODED_IDEAS and hardcoded_ideas:
                    # Use pre-defined idea from ideas.yaml
                    hardcoded_idea = get_hardcoded_idea()
                    llm_response = hardcoded_idea
                    chat_messages.append({"role": "system", "content": "Using hardcoded initial idea."})
                    USE_HARDCODED_IDEAS = False  # Disable hardcoded ideas after first use
                else:
